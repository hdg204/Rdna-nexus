extract_snp=function(chr,bp){
  ...
  if (nrow(data$variants)==0){
    ...
    for (i in 1:nrow(data$data)){ #nrow data$data is the amount of snps picked up at that bp:chr
      colname=paste(chr,':',bp,'_',data$variants$allele0[i],'_',data$variants$allele1[i],sep='')
      mat=data$data[i,,]
      geno=as.numeric(mat[,2]+2*mat[,3])
      genotypes[[colname]]=geno
    }
    
    out=list(snp_data=genotypes,variant_info=data$variants)
    return(out)
  }
}
